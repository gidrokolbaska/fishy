...
class PhotosEditBottomSheetPage extends ConsumerStatefulWidget {
  const PhotosEditBottomSheetPage({
    Key? key,
    required this.imagesUrl,
  }) : super(key: key);
  final List<String> imagesUrl;
  @override
  ConsumerState<PhotosEditBottomSheetPage> createState() =>
      _PhotosBottomSheetPageState();
}

class _PhotosBottomSheetPageState
    extends ConsumerState<PhotosEditBottomSheetPage> {
  late final MultiImagePickerController controller;
  late List<String> localStateImages;
  @override
  void initState() {
    super.initState();
    localStateImages = List.from(widget.imagesUrl);
  ...
  void removeImage(String url) {
    setState(() {
      localStateImages.remove(url);
    });
    ref.read(removedPhotosStateProvider.notifier).update((state) {
      state.add(url);
      return state;
    });
  }
              ...
              children: [
                ...
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    ref
                        .read(selectedOnlinePhotosListProvider.notifier)
                        .update(localStateImages);
                    context.popRoute();
                  },
              ...
              children: [
                      ...
                      GridView(
                        ...
                        gridDelegate:
                            const SliverGridDelegateWithMaxCrossAxisExtent(
                                ...
                                crossAxisSpacing: 31,
                                mainAxisSpacing: 31),
                        children: localStateImages
                            .map<Widget>((e) => UrlImageItem(
                                  url: e,
                ...
                Expanded(
                  child: MultiImagePickerView(
                    ...
                    padding:
                        const EdgeInsets.only(left: 31, right: 31, top: 8.0),
                    draggable: false,
                    gridDelegate:
                        const SliverGridDelegateWithMaxCrossAxisExtent(
                            maxCrossAxisExtent: 94,
                    ...
                    onChange: (p0) {
                      ...
                      } else if (!controller.hasNoImages &&
                          updateOnce == false) {
                        setState(() {
                          updateOnce = true;
                        });
...
class UrlImageItem extends StatelessWidget {
  const UrlImageItem(
      {Key? key,
      required this.url,
...
class PhotoFullScreenView extends StatefulWidget {
  const PhotoFullScreenView({super.key, this.file, this.url});
  final ImageFile? file;
  final String? url;
...
class _PhotoFullScreenViewState extends State<PhotoFullScreenView>
  ...
  Widget build(BuildContext context) {
    return DismissiblePage(
      ...
      disabled: scale > 1,
      child: Hero(
        tag: widget.file?.path ?? widget.url!,
        flightShuttleBuilder: (flightContext, animation, flightDirection,
            fromHeroContext, toHeroContext) {
